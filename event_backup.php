            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-4">
                    <div class="event-meta">
                        ...
                        <div class="event-meta-item">
                            <i class="fas fa-users"></i>
                            <div>Max. <?= $event['max_participants'] ?> účastníků</div>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="event-meta-item">
                            ...
                            <div><?= number_format($event['price'], 0, ',', ' ') ?> Kč</div>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="row">
                    ...
                    <div class="event-content">
                        ...
                        <?php if ($event['registration_required']): ?>
                            <div class="registration-section">
                                <h3><i class="fas fa-clipboard-list me-2"></i>Registrace nutná</h3>
                                <p>Pro účast na této akci je vyžadována předchozí registrace.</p>
                                
                                <?php if ($event['registration_deadline']): ?>
                                    <p><strong>Deadline registrace:</strong> <?= date('j.n.Y', strtotime($event['registration_deadline'])) ?></p>
                                <?php endif; ?>
                                
                                <div class="mt-3">
                                    <a href="mailto:<?= htmlspecialchars($settings['contact_email'] ?? 'info@pohoda-antosovice.cz') ?>?subject=Registrace na akci: <?= urlencode($event['title']) ?>" class="btn btn-register">
                                        <i class="fas fa-envelope me-2"></i>Zaregistrovat se
                                    </a>
                                </div>
                            </div>
                        <?php endif; ?>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="sidebar-widget">
                        <h4><i class="fas fa-newspaper me-2"></i>Související články</h4>
                        
                        <?php foreach ($relatedPosts as $post): ?>
                        <div class="related-item">
                            <h6><a href="post_new.php?slug=<?= htmlspecialchars($post['slug']) ?>"><?= htmlspecialchars($post['title']) ?></a></h6>
                            <small class="text-muted">
                                <i class="fas fa-calendar me-1"></i>
                                <?= date('j.n.Y', strtotime($post['created_at'])) ?>
                            </small>
                            <?php if ($post['excerpt']): ?>
                                <p class="mt-2"><?= htmlspecialchars(substr($post['excerpt'], 0, 100)) ?>...</p>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <?php if (!empty($similarEvents)): ?>
                    <div class="sidebar-widget">
                        <h4><i class="fas fa-calendar-plus me-2"></i>Další akce</h4>
                        
                        <?php foreach ($similarEvents as $similarEvent): ?>
                        <div class="related-item">
                            <h6><a href="event.php?slug=<?= htmlspecialchars($similarEvent['slug']) ?>"><?= htmlspecialchars($similarEvent['title']) ?></a></h6>
                            <small class="text-muted">
                                <i class="fas fa-calendar me-1"></i>
                                <?= date('j.n.Y', strtotime($similarEvent['start_date'])) ?>
                                <?php if ($similarEvent['start_time']): ?>
                                    <?= date('H:i', strtotime($similarEvent['start_time'])) ?>
                                <?php endif; ?>
                            </small>
                            <?php if ($similarEvent['location']): ?>
                                <br><small class="text-muted">
                                    <i class="fas fa-map-marker-alt me-1"></i>
                                    <?= htmlspecialchars($similarEvent['location']) ?>
                                </small>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
